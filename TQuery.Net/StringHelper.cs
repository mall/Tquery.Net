...
namespace TQuery.Net
{
    public static class StringHelper
    {
        ...
        /// <param name="splitMark">Html编辑器里面的分页标记</param>
        public static String[] ArticlePage(string content, int currentPage, string pageUrl, string splitMark)
        {
            String[] result = new String[2];
            int pageCount = 0;//页数        
            content = content.Replace(splitMark, "[--page--]");//FCK在IE中生成的默认分页符，替换为自定义分页符        
            ...
            else
            {
                ...
                if (currentPage != 1)
                {
                    pageStr += "<a class='prev' href =" + pageUrl + "-" + (currentPage - 1) + "><<上一页</a>";
                }
                for (int i = 1; i <= pageCount; i++)
                {
                    if (i == currentPage)
                        ...
                        pageStr += ("<a class='num' href =" + pageUrl + "-" + i + ">[" + i + "]</a>");
                }
                if (currentPage != pageCount)
                {
                    pageStr += "<a class='next' href =" + pageUrl + "-" + (currentPage + 1) + ">下一页>></a>";
                }
                result[0] = pageStr;
